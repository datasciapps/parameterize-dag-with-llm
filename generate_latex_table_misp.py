#!/usr/bin/env python3
"""
Generate LaTeX table from combined misspecification aggregated statistics with 95% CI.
"""
...
# Misspecification DAG edge mapping
DAG_MAP = {
    "exp_sp_owner_expenditure": "Owner → Expenditure",
    "exp_sp_majorcards_dependents": "MajorCards → Dependents",
    "exp_sp_owner_share": "Owner → Share",
    "exp_sp_majorcards_selfemp": "MajorCards → SelfEmp",
}

def parse_label(label):
    """Extract DAG and model from label like 'exp_sp_owner_expenditure_llama31_25_5'"""
    parts = label.split('_')
    # Reconstruct DAG code from parts (exp_sp_owner_expenditure, etc.)
    dag_code = '_'.join(parts[:4])  # exp_sp_owner_expenditure
    model_code = parts[4]  # llama31
...
def main():
    # Read the misspecification combined CSV
    input_file = Path("output/missp_combined_aggregated_stats.csv")
    ...
    dag_order = ["Owner → Expenditure", "MajorCards → Dependents", "Owner → Share", "MajorCards → SelfEmp"]
    ...
    # Save to CSV
    output_csv = Path("output/misp_latex_table_stats.csv")
    results_df.to_csv(output_csv, index=False)
    print(f"✓ Saved CSV to: {output_csv}")
    
    # Generate LaTeX table rows
    output_latex = Path("output/misp_latex_table_stats.tex")
    with open(output_latex, 'w') as f:
        for _, row in results_df.iterrows():
            latex_row = (
                f"{row['Model']:<20} & {row['DAG']:<25} & "
                f"{row['L2 Norm']} & "
