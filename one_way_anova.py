...
def extract_key_value(directory_path, key_column):
    ...
    for file_path in file_list:
        try:
            ...
            if key_column not in df.columns:
                print(f"Warning: Column '{key_column}' not found in {file_path}. Skipping.")
                continue
            ...
            if pd.api.types.is_numeric_dtype(type(value)):
                key_values.append(value)
                 ...
                 print(f"Warning: Column value ({value}) in {file_path} is not numeric. Skipping.")
    ...
    print(f"Found {len(key_values)} valid '{key_column}' values using pattern '{FILENAME_PATTERN}'.")
    print("-" * (len(directory_path) + 30))
    return key_values


def perform_anova(data_a, data_b, data_c, key_column, condition_names=["Condition A", "Condition B", "Condition C"]):
    """
    ...
    if not data_a or not data_b or not data_c:
        print("\n--- ANOVA Canceled ---")
        print("ANOVA requires at least one data point for all groups. Check the extraction process.")
        return
    ...
    try:
        ...
        if p_value < alpha:
            ...
            print(f"**Conclusion: We reject the null hypothesis.** There is a statistically significant difference in the '{key_column}' means across the three conditions.")
        else:
            print(f"\nThe P-value ({p_value:.4f}) is greater than the significance level (alpha={alpha}).")
            print(f"**Conclusion: We fail to reject the null hypothesis.** There is no statistically significant difference in the '{key_column}' means across the three conditions.")

    except ValueError as e:
        print(f"Error during ANOVA calculation (likely due to insufficient or constant data): {e}")
...
def main():
    ...
    print(f"Starting ANOVA script with column='{key_column}' and expected N={expected_n}.")
    ...
    for name, data in data_groups.items():
        current_n = len(data)
        try:
            assert current_n == expected_n, f"Sample size mismatch in {name}. Expected N={expected_n}, but found N={current_n}."
    ...
    for name, data in data_groups.items():
        if data:
            print(f"{name} (N={len(data)}): Mean={np.mean(data):.4f}, Std Dev={np.std(data):.4f}")
        else:
             print(f"{name}: No data extracted.")

    # 4. Perform One-Way ANOVA
    perform_anova(l2_norms_a, l2_norms_b, l2_norms_c, key_column, list(condition_dirs.keys()))
...
if __name__ == "__main__":
    try:
        ...
        print("\n*** Script Halted ***")
        print(f"The ANOVA calculation was skipped due to the sample size assertion failure: {ae}")
    except Exception as e:
