...
def split_equation(equation_str_to_split, verbose=False): # Renamed for clarity
  if verbose:
    print(f"Equation part for splitting (after error term removal): {equation_str_to_split}")
  ...
  split_parts = [p.strip() for p in split_parts if p.strip()] # Remove any empty strings resulting from the split
  ...
  if split_parts[0] in ['+', '-']:
      ...
      if len(split_parts) > 1:
          ...
          if verbose: print(f"Warning: Malformed equation string '{equation_str_to_split}' resulted in only an operator: {split_parts[0]}")
  ...
  while current_term_idx < len(split_parts):
      ...
      if current_term_idx + 1 < len(split_parts):
          ...
          if verbose:
              print(f"Warning: Trailing operator '{operator}' detected at the end of equation part: {equation_str_to_split}. Ignoring.")
      current_term_idx += 2
...
def split_equations_to_terms(llm_responses_df: pd.DataFrame, target_variable_name: str, verbose=False) -> list[list[str]]:
  ...
  for i, lin_eq_raw in enumerate(llm_responses_df['proposed_lin_str_eq'].to_list()):
    ...
    if isinstance(lin_eq_raw, dict):
      lin_eq = lin_eq_raw.get('equation', '') # Extract the 'equation' string if it's a dict
      assert lin_eq, f"Index {i}: Dictionary found but 'equation' key missing or empty: {lin_eq_raw}"
      ...
      try:
        ...
        assert False, f"Index {i}: Raw string is a complete JSON object, expected equation string: {lin_eq_raw}"
      except json.JSONDecodeError:
        # If it's not a complete JSON object, then it might be a valid equation string or contain nested JSON.
        lin_eq = lin_eq_raw
    else:
      assert False, f"Index {i}: Expected string or dict, got {type(lin_eq_raw).__name__}: {lin_eq_raw}"
    ...
    if not error_term_match:
        ...
        assert False, f"Index {i}: No error term (e.g., E_Y or E_VARIABLE) found in equation: {lin_eq_raw}"
    ...
    equation_cleaned_rhs = re.sub(r'(\s*(?<![\d.])\+\s*|\s*(?<![\d.])-\s*)$', '', equation_cleaned_rhs).strip()
