...
class PromptPerNode:
    def __init__(self, primary_domain_name: str, secondary_domain_name: str, target_variable_name: str, direct_parent_variables: list[str], node_descriptions: dict = None, node_lower_bounds: dict = None, node_upper_bounds: dict = None, include_constraints_in_prompt: bool = False, feedback_message: str = None, phenomenon_overview: str = None):
        ...
        self.node_descriptions = node_descriptions if node_descriptions is not None else {}
        self.node_lower_bounds = node_lower_bounds if node_lower_bounds is not None else {}
        self.node_upper_bounds = node_upper_bounds if node_upper_bounds is not None else {}
        ...
        self.phenomenon_overview = phenomenon_overview # New parameter for phenomenon overview
    ...
    def _generate_prompt_template(self) -> str:
        ...
        if self.include_constraints_in_prompt:
            ...
            if t_lb is not None and t_ub is not None:
                constraints_list.append(f"- Variable {self.target_variable_name} is bounded within [{t_lb}, {t_ub}].")
            ...
            for var in self.direct_parent_variables:
                ...
                if p_lb is not None and p_ub is not None:
                    constraints_list.append(f"- Variable {var} is bounded within [{p_lb}, {p_ub}].")
        ...
        prompt_template = r"""Given the direct causes, you must propose a linear structural equation for the target variable $Y$. Do not use non-linear functions (e.g., exponential, sigmoid). The coefficients of the linear equation are continuous variables in space $\mathbb{R}$.
...
{feedback_text}""".replace("{target_variable_formatted}", target_variable_formatted).replace("{direct_causes_formatted}", direct_causes_formatted).replace("{primary_domain_name_lower}", self.primary_domain_name.lower()).replace("{hard_constraints_text}", hard_constraints_text).replace("{target_variable_name}", self.target_variable_name).replace("{feedback_text}", feedback_text)

        # Now using actual variable names for equation terms and betas
        equation_terms = " + ".join([f"\\beta_{var_name}*{var_name}" for var_name in self.direct_parent_variables])
        prompt_template = prompt_template.replace("{equation_terms}", equation_terms)

        betas = ", ".join([f"\\beta_{var_name}" for var_name in self.direct_parent_variables])
    ...
    def _generate_output_constraint_prompt(self) -> str:
        plausibility_description = f"{self.primary_domain_name.capitalize()} Plausibility"
...
""".format(plausibility_description=plausibility_description)
    ...
    def get_full_prompt(self) -> str:
        return self.get_expert_profile_specification() + self.prompt_template + self.output_constraint_prompt
    ...
    def visualize_parent_child_relationship(self, effect_sizes: dict = None):
        ...
        for parent in self.direct_parent_variables:
            if effect_sizes and parent in effect_sizes:
                ...
                dot_string += f"    \"{parent}\" -> \"Y: {self.target_variable_name}\" [{label}];\n"
            else:
