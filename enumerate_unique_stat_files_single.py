...
def main():
    ...
    # Build reverse index: stat_id -> logs that contain that stat save line.
    # This is important for looped logs where filename contains a parent run ID
    # (e.g., 20260320_165918_...) but child stat IDs are inside the log body.
    stat_id_to_logs = {}
    ...
    for log in log_files:
        try:
            with open(log, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            for sid in set(stat_file_pattern.findall(content)):
                stat_id_to_logs.setdefault(sid, []).append(log)
        except Exception:
            continue
    ...
    for exp_id, files in sorted(id_to_files.items()):
        # Find logs that match by filename OR by explicit stat save lines in body.
        filename_matched_logs = [log for log in log_files if exp_id in log.name.replace('_','')]
        body_matched_logs = stat_id_to_logs.get(exp_id, [])
        matching_logs = list({*filename_matched_logs, *body_matched_logs})
                    ...
                    with open(log, 'r', encoding='utf-8', errors='ignore') as f:
                        ...
                        for idx, line in enumerate(lines):
                            ...
                            if 'Completed successfully' in line:
                                ...
                                # Count/report only success lines that belong to this exp_id.
                                # Fallback: if no stat_id captured, allow filename-based association.
                                if stat_id == exp_id or (not stat_id and exp_id in log.name.replace('_', '')):
                                    success_count += 1
