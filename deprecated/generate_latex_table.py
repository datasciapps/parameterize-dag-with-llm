...
def parse_label(label):
    """Extract DAG and model from label like 'foo_llama31_25_5'"""
    parts = label.split('_')
    dag_code = parts[0]
    model_code = parts[1]
...
def main():
    ...
    input_file = Path("output/combined_aggregated_stats.csv")
    ...
    dag_order = ["foodsecurity", "algal2", "lexical", "liquefaction", "stocks"]
    ...
    # Save to CSV
    output_csv = Path("output/latex_table_stats.csv")
    results_df.to_csv(output_csv, index=False)
    print(f"✓ Saved CSV to: {output_csv}")
    
    # Generate LaTeX table rows
    output_latex = Path("output/latex_table_stats.tex")
